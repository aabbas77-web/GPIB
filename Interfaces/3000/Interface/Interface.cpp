...
void __fastcall TFormInterface::Measure(TStringList *List)
{
 ...
 int Back_Size;
 int Error=PM3000.pm3000_meas_read_back_data(PM3000.instr_ID,InParams->Params[8]->Index,100,InParams->Params[8]->Index,Back_Data_Array,Back_Size);
 if(Error!=0)
  {
   Stop();
   ...
   for(int i=0;i<ComboBox1->Items->Count;i++)
    {
     ...
     if(ListBox1->Items->IndexOf(Name)>=0)
      {
       Table1->FieldByName(Name)->AsString=FormatFloat("0.000",Back_Data_Array[k]);
       if(ComboBox4->Text==Name)
        Value=Back_Data_Array[k];
       k++;
      }
    }
  }
   ...
   for(int i=0;i<ComboBox1->Items->Count;i++)
    {
     ...
     if(ListBox1->Items->IndexOf(Name)>=0)
      {
       List->Add(Name+" = "+FormatFloat("0.000",Back_Data_Array[k]));
       k++;
      }
    }
  }
 ...
 int Fore_Size;
 Error=PM3000.pm3000_meas_read_fore_data (PM3000.instr_ID, InParams->Params[8]->Index, Fore_Data_Array, Fore_Size);
 if(Error!=0)
  {
   Stop();
 ...
 if(!IsSingle)
  {
   ...
   for(int i=0;i<ComboBox2->Items->Count;i++)
    {
     AnsiString Name=ComboBox2->Items->Strings[i];
     if(ListBox1->Items->IndexOf(Name)>=0)
      {
       Table1->FieldByName(Name)->AsString=FormatFloat("0.000",Fore_Data_Array[k]);
       if(ComboBox4->Text==Name)
        Value=Fore_Data_Array[k];
       k++;
      }
    }
  }
   ...
   for(int i=0;i<ComboBox2->Items->Count;i++)
    {
     ...
     if(ListBox1->Items->IndexOf(Name)>=0)
      {
       List->Add(Name+" = "+FormatFloat("0.000",Fore_Data_Array[k]));
       k++;
      }
    }
  }
 ...
 if(ComboBox3->Text!="")
  {
   ...
   int Error=PM3000.pm3000_meas_read_fund_data (PM3000.instr_ID, InParams->Params[8]->Index, ComboBox3->ItemIndex, fun_value);
   if(Error!=0)
    {
     Stop();
